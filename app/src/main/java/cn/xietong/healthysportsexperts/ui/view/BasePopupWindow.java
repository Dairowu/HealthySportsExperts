...
import cn.xietong.healthysportsexperts.interfacer.BasePopup;
import cn.xietong.healthysportsexperts.utils.LogUtils;


/**
 * Created by 林思旭 on 2016/8/14.
 * 抽象通用popupwindow的父类
 */
public abstract class BasePopupWindow implements BasePopup {
    private final String TAG = "BasePopupWindow";
    private SlideFromBottomPopup.MyPupopBroadcastReceiver myreceiver;//10.31
    private IntentFilter intentFilter;
    public static String SETDATA = "SlideFromBottomPopup";
    private TextView txt_time,txt_distance;
    private String time="00:00",distance = null,destroy = null;
    private final int DATE_TIME = 1,DATE_DISTANCE = 2,DATE_CALORIE = 3,DTAE_DESTROY = 4;
    private SetTimeHandler timeHandler;//10.31
    //元素定义
    protected PopupWindow mPopupWindow;
//    protected PopupWindow mTopPopupWindow;
    //popup视图
//    protected View mTopPopupView;//Magic
    protected View mPopupView;
    protected View mAnimaView;
    protected View mTopAnimaView;//Magic
    protected View mDismissView;
//    protected View mDismissTopView;//Magic
    ...
    protected Animation topAnima;//Magic
    ...
    private void initView(Activity context, int w, int h) {
        mContext = context;
//        mTopPopupView = getTopPopupView();//获取顶部视图,Magic
        mPopupView = getPopupView();//获取视图

        txt_time = (TextView)mPopupView.findViewById(R.id.top_popup_time);//Magic,206.10.31
        txt_distance = (TextView)mPopupView.findViewById(R.id.top_popup_distance);//2016.10.31
//        mTopPopupView.setFocusableInTouchMode(true);//Magic
        mPopupView.setFocusableInTouchMode(true);
        //默认占满全屏
//        mTopPopupWindow = new PopupWindow(mTopPopupView,w,h);//Magic
        mPopupWindow = new PopupWindow(mPopupView, w, h);
        //Magic
//        mTopPopupWindow.setBackgroundDrawable(new ColorDrawable());
//        mTopPopupWindow.setFocusable(true);
...
//        mTopPopupWindow.setAnimationStyle(R.style.PopupAnimaFade);//Magic
        //=============================================================为外层的view添加点击事件，并设置点击消失
        mAnimaView = getAnimaView();//获取需要添加动画的View
        mTopAnimaView = getTopAnimaView();//Magic
        mDismissView = getClickToDismissView();//设置点击效果
//        mDismissTopView = getClickToTopDismissView();//Magic
        if (mDismissView != null) {
            mDismissView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    dismiss();
//                    mContext.unregisterReceiver(myreceiver);//2016.10.31
                    LogUtils.i(TAG,"点击");
                }
            });
            if (mAnimaView != null) {
        ...
        initMyPopopBoardcastReceiver(mContext);//注册广播
        timeHandler = new SetTimeHandler();//10.31
        //=============================================================元素获取
        curAnima = getShowAnimation();
        topAnima = getShowTopAnimation();
     ...
     * @return
     */
    protected abstract Animation getShowTopAnimation();
    /**
     ...
     * @return
     */
//    protected abstract View getClickToTopDismissView();
    /**
     * 设置展示动画View的属性动画
     * @return
     */
...
//            mTopPopupWindow.showAtLocation(v, Gravity.CENTER, 0, 0);//Magic
        }
        //传递了res
        if (res != 0 && v == null) {
            mPopupWindow.showAtLocation(mContext.findViewById(res), Gravity.CENTER, 0, 0);
//            mTopPopupWindow.showAtLocation(mContext.findViewById(res), Gravity.CENTER, 0, 0);//Magic
        }
        //什么都没传递，取顶级view的id
        if (res == 0 && v == null) {
            mPopupWindow.showAtLocation(mContext.findViewById(android.R.id.content), Gravity.CENTER, 0, 0);
//            mTopPopupWindow.showAtLocation(mContext.findViewById(android.R.id.content), Gravity.CENTER, 0, 0);//Magic
        }
        if (curAnima != null && mAnimaView != null) {
            mAnimaView.clearAnimation();
            mTopAnimaView.clearAnimation();
//            mTopAnimaView.clearAnimation();//Magic
            mAnimaView.startAnimation(curAnima);
            mTopAnimaView.startAnimation(topAnima);//Magic
        }
        ...
        try {
            if (curExitAnima != null && mAnimaView != null) {
                if (!isExitAnimaPlaying) {
                    ...
                    mTopAnimaView.startAnimation(curExitAnima);//Magic
...
//                mTopPopupWindow.dismiss();
     ...
     * 注册广播(2016.10.31)
     */
    private void initMyPopopBoardcastReceiver(Context context){
        myreceiver = new MyPupopBroadcastReceiver();
        intentFilter = new IntentFilter();
        intentFilter.addAction(BasePopupWindow.SETDATA);
        context.registerReceiver(myreceiver,intentFilter);
    }

    /**
     * 用来接受后台运行的数据,2016.10.31
     */
    class MyPupopBroadcastReceiver extends BroadcastReceiver {
        @Override
...
//                timeHandler.sendMessage(message);
    ...
    /**
     * 用来设置所有有关顶部冒泡窗口视图的参数,2016.10.31
     */
    class SetTimeHandler extends Handler{
        @Override
