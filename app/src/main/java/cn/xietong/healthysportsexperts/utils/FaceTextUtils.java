...
//	}
	...
	public static SpannableString toSpannableString(Context context, String text) {
		if (!TextUtils.isEmpty(text)) {
			...
			int start = 0;
			Pattern pattern = Pattern.compile("\\\\ue[a-z0-9]{3}", //字符串a到z 0到9的组成，3位3位数
					Pattern.CASE_INSENSITIVE);//Pattern.CASE_INSENSITIVE：不分大小写模式
			Matcher matcher = pattern.matcher(text);
			while (matcher.find()) {
				String faceText = matcher.group(); //获取compile()里全部目标对象
